...
class DataLoader():
    ...
    def preprocessing(self, file_path, mode="ml-1m"):
        ...
        ## extract short sequence (B) #######
        df[['short_product_id', 'short_rating', 'sample_product_id', 'sample_rating']] = df.apply(lambda x: self.extract_short_sequence(x.product_id, x.rating), axis=1)
        df = df[['user_id', 'sample_product_id', 'sample_rating']]
        df = df.rename(columns = {'sample_product_id':'product_id', 'sample_rating':'rating'})
        #####################################
        # step_size = 1
    ...
    ####################################################
    ## create_sequences edited
    def create_sequences(self, values, step_size=1): 
        sequences = []
        pad = [0] * self.max_seq_length
        for window_size in range(2, max_seq_length = self.max_seq_length+1): ##edit
            start_index = 0
            while True:
    ...
    def extract_short_sequence(self, product_id, rating):
        start_id = np.random.randint(len(product_id)-self.max_seq_length)

        B_product = product_id[start_id: start_id+self.max_seq_length]
        B_rating = rating[start_id: start_id+self.max_seq_length]

        C_product = self.create_sequences(B_product)
        C_rating = self.create_sequences(B_rating)

        return pd.Series([B_product, B_rating, C_product, C_rating])
